...
function updateMenuStreak() {
    ...
    for (let i = startDay; i < startDay + 7; i++) {
        ...
        const opacity = i === streak ? "1" : "0.5";
        streakHtml += `<div style="width: 24px; height: 24px; background: ${color}; border: ${border}; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; opacity: ${opacity}; color: var(--dark1); font-size: 0.7rem;">${i}</div>`;
    }
    streakHtml += `</div>`;

    document.getElementById("menu-streak").innerHTML = `
        <div style="color: var(--light2); font-size: 0.9rem; text-align: center;">
...
function exploreCombatWild() {
    ...
    if (barProgressWild < 99) {
        ...
        if (team[exploreActiveMember].item == item.eviolite.id && pkmn[team[exploreActiveMember].pkmn.id].evolve !== undefined && (pkmn[team[exploreActiveMember].pkmn.id].evolve()[1].pkmn.id.slice(0, 4) !== "mega" || team[exploreActiveMember].pkmn.id == pkmn.bayleef.id)) { totalPower /= item.eviolite.power(); }
...
function initialiseArea() {
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3c4.97 0 9 4.03 9 9"><animateTransform attributeName="transform" dur="1.5s" repeatCount="indefinite" type="rotate" values="0 12 12;360 12 12"/></path></svg>
           ...
           <div> Auto-Refight is active!<br>Click to disable it</div>
    `
    }
...
function claimWonderTrade() {
    ...
    if (pkmn.arceus.caught > 0) poolInfo = `Pool: 100% Duplicate (Collection Complete)`

    let historyHtml = ""
    if (saved.wonderTradeLog && saved.wonderTradeLog.length > 0) {
        historyHtml = `<div style="margin-top: 1rem; border-top: 1px solid var(--light1); padding-top: 0.5rem; width: 100%;">
            <div style="font-size: 0.8rem; margin-bottom: 0.5rem; opacity: 0.8">Recent Trades:</div>
            <div style="display: flex; justify-content: center; gap: 5px; flex-wrap: wrap;">`
        saved.wonderTradeLog.forEach(entry => {
            const spritePath = entry.receivedShiny ? `img/pkmn/shiny/${entry.received}.png` : `img/pkmn/sprite/${entry.received}.png`
            const sentPath = `img/pkmn/sprite/${entry.sent || 'magikarp'}.png`
            historyHtml += `
            <div style="display: flex; align-items: center; gap: 2px; background: var(--dark2); padding: 2px; border-radius: 4px; border: 1px solid var(--light1)">
                <img src="${sentPath}" style="width: 24px; height: 24px; image-rendering: pixelated; opacity: 0.6" title="Sent ${format(entry.sent || 'magikarp')}">
                <span style="font-size: 10px; opacity: 0.5">→</span>
                <img src="${spritePath}" style="width: 32px; height: 32px; image-rendering: pixelated;" title="Received ${format(entry.received)}">
            </div>`
        })
        historyHtml += `</div></div>`
    }
...
function wonderTrade() {
    ...
    document.getElementById("wonder-text").innerHTML = `You traded your ${format(sentPokemon)} for a ${format(chosenPokemon)}!`
    ...
    // Log the trade
    saved.wonderTradeLog ??= []
    saved.wonderTradeLog.unshift({
        sent: sentPokemon,
        received: chosenPokemon,
        receivedShiny: chosenShiny,
        timestamp: Date.now()
    })
    if (saved.wonderTradeLog.length > 5) saved.wonderTradeLog.pop()
...
function checkDailyLogin() {
    ...
    if (saved.lastLoginDay === 0) {
        saved.lastLoginDay = dayNumber;
        saved.loginStreak = 1;
...
function showDailyLoginReward() {
    ...
    for (let i = startDay; i < startDay + 7; i++) {
        ...
        const border = active ? "2px solid var(--light1)" : "1px solid var(--light1)";
        const opacity = i === streak ? "1" : "0.5";
        streakHtml += `<div style="width: 30px; height: 30px; background: ${color}; border: ${border}; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; opacity: ${opacity}; color: var(--dark1); font-size: 0.8rem;">${i}</div>`;
    }
    streakHtml += `</div>`;

    document.getElementById("tooltipMid").innerHTML = `
        Welcome back! You're on a <strong>${streak} day</strong> streak.<br>
        Today's Reward: <strong>x${reward.amount} ${reward.name}</strong>
        ${streakHtml}
    `;
...
function claimDailyLoginReward(streak) {
    ...
    console.info(`Claimed daily reward: x${reward.amount} ${reward.name}`);
}
