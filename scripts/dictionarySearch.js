...
function setSearchTags() {
    ...
    for (const e in pkmn) {
        ...
        refreshPkmnSortTags(e)
    }
}


function refreshPkmnSortTags(id) {
    if (!pkmn[id]) return;
    ...
    stats.forEach(s => {
        let stars = pkmn[id].bst?.[s] || 0;
        if (pkmn[id].nature && nature[pkmn[id].nature] && nature[pkmn[id].nature][s]) {
            stars += nature[pkmn[id].nature][s];
        }
        const baseVal = stars * 30;
        const totalVal = Math.round(returnPkmnStatValue(pkmn[id], s));

        totalBst += baseVal;
        totalIvBonus += (totalVal - baseVal);
    });

    pkmn[id].dictionaryTagBstSum = totalBst;
    pkmn[id].dictionaryTagIvSum = totalIvBonus;
}
